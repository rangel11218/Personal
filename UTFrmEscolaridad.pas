...
type
  TFrmEscolaridad = class(TForm)
    cdEscolaridad: TZQuery;
    tvDatos: TcxGridDBTableView;
    cxGrid1Level1: TcxGridLevel;
    cxGrid1: TcxGrid;
    dsEscolaridad: TDataSource;
    ColCodigoEscolaridad: TcxGridDBColumn;
    ColTituloEscolaridad: TcxGridDBColumn;
    Panel1: TPanel;
    ...
    CodigoEscolaridad: TDBEdit;
    TituloEscolaridad: TDBEdit;
    JvLabel1: TJvLabel;
    JvLabel2: TJvLabel;
    Panel2: TPanel;
...
var
  FrmEscolaridad: TFrmEscolaridad;

implementation

{$R *.dfm}

procedure TFrmEscolaridad.UpdateRecord(Modo: TDataSetState);
var
  FormaEdit: TForm;
begin
  ...
  try
    if Modo = dsEdit then
      FormaEdit.Caption := 'Editar Escolaridad'
    else
      FormaEdit.Caption := 'Nuevo Escolaridad';
...
procedure TFrmEscolaridad.btnAgregarClick(Sender: TObject);
begin
  UpdateRecord(dsInsert);
end;

procedure TFrmEscolaridad.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmEscolaridad.btnEditarClick(Sender: TObject);
begin
  UpdateRecord(dsEdit);
end;

procedure TFrmEscolaridad.FormaEditShow(Sender: TObject);
begin
  CodigoEscolaridad.SetFocus;
end;

procedure TFrmEscolaridad.FormaEditCloseQuery(Sender: TObject;
  ...
  begin
    ...
    try
      ...
      try
        ...
        if Trim(CodigoEscolaridad.Text) = '' then
        begin
          Obj := CodigoEscolaridad;
          raise InteligentException.CreateByCode(18, ['Código']);
        end;

        if Trim(TituloEscolaridad.Text) = '' then
        begin
          Obj := TituloEscolaridad;
          raise InteligentException.CreateByCode(18, ['Título']);
        end;

        cdEscolaridad.Post;
        cdEscolaridad.Refresh;
      ...
      on e:Exception do
      begin
        if cdEscolaridad.State in [dsEdit, dsInsert] then
          cdEscolaridad.Cancel;
...
procedure TFrmEscolaridad.FormShow(Sender: TObject);
var
  LocCursor: TCursor;
begin
  try
    ...
    try
      ...
      CrearConjunto(cdEscolaridad, 'rhu_escolaridad', ccUpdate);
      cdEscolaridad.Open;
    finally
      Screen.Cursor := LocCursor;
    end;
