...
begin
  ...
  repeat
    {Copy this row}
    Dest^ := Char(fOwner.GammaTable[pByte(Longint(Src) + 2)^]); inc(Dest);
    Dest^ := Char(fOwner.GammaTable[pByte(Longint(Src) + 1)^]); inc(Dest);
    Dest^ := Char(fOwner.GammaTable[pByte(Longint(Src)    )^]); inc(Dest);
...
begin
  ...
  repeat
    {Copy this row}
    Dest^ := Char(Owner.GammaTable[pByte(Longint(Src) + 4)^]); inc(Dest);
    Dest^ := Char(Owner.GammaTable[pByte(Longint(Src) + 2)^]); inc(Dest);
    Dest^ := Char(Owner.GammaTable[pByte(Longint(Src)    )^]); inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
    Extra^ := Char(fOwner.GammaTable[pByte(Longint(Src) + 5)^]); inc(Extra);
    Extra^ := Char(fOwner.GammaTable[pByte(Longint(Src) + 3)^]); inc(Extra);
    Extra^ := Char(fOwner.GammaTable[pByte(Longint(Src) + 1)^]); inc(Extra);
    {$ENDIF}
...
begin
  ...
  repeat
    ...
    repeat
      ...
      {Copy data}
      {Dest2^ := Byte(Dest2^) or (((Byte(Src^) shr CurBit) and $3)
         shl (4 - (4 * Col) mod 8));}
      {Move to next column}
      inc(Col, ColumnIncrement[Pass]);
      {Will read next bits}
...
begin
  ...
  repeat
    ...
    Dest^  := Char(fOwner.GammaTable[pByte(Longint(Src) + 4)^]); inc(Dest);
    Dest^  := Char(fOwner.GammaTable[pByte(Longint(Src) + 2)^]); inc(Dest);
    Dest^  := Char(fOwner.GammaTable[pByte(Longint(Src)    )^]); inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
...
begin
  ...
  begin
    Dest^ := Char(((Byte(Src^) shr 2) and $F) or ((Byte(Src^)) and $F0));
      inc(Dest);
    Dest^ := Char(((Byte(Src^) shl 2) and $F) or ((Byte(Src^) shl 4) and $F0));
      inc(Dest);
    inc(Src);
  end {FOR i}
end;
...
begin
  ...
  FOR i := 1 TO Row_Bytes do
  begin
    Dest^ := Char(((Byte(Src^) shr 4) and $3) or ((Byte(Src^) shr 2) and $30));
      inc(Dest);
    Dest^ := Char((Byte(Src^) and $3) or ((Byte(Src^) shl 2) and $30));
      inc(Dest);
    inc(Src);
  end {FOR i}
end;
...
begin
  ...
  begin
    {Copy pixel values}
    Dest^ := Char(fOwner.InverseGamma[pByte(Longint(Src) + 2)^]); inc(Dest);
    Dest^ := Char(fOwner.InverseGamma[pByte(Longint(Src) + 1)^]); inc(Dest);
    Dest^ := Char(fOwner.InverseGamma[pByte(Longint(Src)    )^]); inc(Dest);
    {Move to next pixel}
    inc(Src, 3);
  end {for I}
end;
...
begin
  ...
  FOR i := 1 TO ImageWidth do
  begin
    Dest^ := Char(Owner.InverseGamma[PByte(Longint(Src) + 2)^]); inc(Dest);
    Dest^ := Char(Owner.InverseGamma[PByte(Longint(Src) + 1)^]); inc(Dest);
    Dest^ := Char(Owner.InverseGamma[PByte(Longint(Src)    )^]); inc(Dest);
    Dest^ := Trans^; inc(Dest);
    inc(Src, 3); inc(Trans);
  end {for i};
end;
