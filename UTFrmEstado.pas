...
type
  TFrmEstado = class(TForm)
    ...
    eCodigoEstado: TDBEdit;
    eTituloEstado: TDBEdit;
    cdDatos: TZQuery;
    dsDatos: TDataSource;
    cdPais: TZReadOnlyQuery;
    dsPais: TDataSource;
    cbbIdPais: TDBLookupComboBox;
    JvLabel3: TJvLabel;
...
procedure TFrmEstado.btnAddClick(Sender: TObject);
begin
  ...
  try
    ...
    cdDatos.Append;
    cdDatos.FieldByName('IdEstado').AsInteger := 0;
...
procedure TFrmEstado.btnBorrarClick(Sender: TObject);
begin
  if InteliDialog.ShowModal('Eliminar registro', 'Ha solicitado eliminar el regsitro:' + #10 + #10 + cdDAtos.FieldByName('TituloEstado').AsString + #10 + #10 + '¿Está seguro de eliminarlo en este momento?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    cdDatos.Delete;
end;
...
procedure TFrmEstado.cdDatosAfterClose(DataSet: TDataSet);
begin
  EstadoBotones;
end;

procedure TFrmEstado.cdDatosAfterDelete(DataSet: TDataSet);
begin
  EstadoBotones;
end;

procedure TFrmEstado.cdDatosAfterOpen(DataSet: TDataSet);
begin
  EstadoBotones;
end;

procedure TFrmEstado.cdDatosAfterPost(DataSet: TDataSet);
begin
  EstadoBotones;
end;

procedure TFrmEstado.FormaEditCloseQuery(Sender: TObject; var CanClose: Boolean);
var
  Obj: TWinControl;
begin
...
procedure TFrmEstado.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
...
begin
  try
    ...
    try
      ...
      CrearConjunto(cdDatos, 'nuc_estado', ccUpdate);
...
procedure TFrmEstado.tvDatosCellDblClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TFrmEstado.EstadoBotones;
begin
  btnAdd.Enabled := True;
...
procedure TFrmEstado.PropiedadesEdit;
begin
