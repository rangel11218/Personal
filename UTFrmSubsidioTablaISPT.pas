...
uses
  ...
  Dialogs, UInteliDialog, JvLabel, StdCtrls, ExtCtrls, Grids, DBGrids, DB,
  StrUtils, DBCtrls, AdvGlowButton, ComCtrls, ToolWin, Spin, Mask, dxSkinsCore,
  ...
  dxSkinOffice2013DarkGray, ZDataset, dxSkinOffice2013LightGray, frxClass,
  frxDBSet, ZAbstractRODataset, UDosCapas, dxSkinOffice2016Colorful,
  dxSkinOffice2016Dark, dxSkinVisualStudio2013Blue, dxSkinVisualStudio2013Dark,
  dxSkinVisualStudio2013Light, cxGraphics, cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, cxContainer, cxEdit, dxCore, cxDateUtils, cxCalc,
  cxTextEdit, cxMaskEdit, cxDropDownEdit, cxCalendar, Vcl.Tabs;
...
type
  TFrmSubsidioTablaISPT = class(TForm)
    ...
    cdTipoTablaISR: TZReadOnlyQuery;
    eFechaAplicacion: TcxDateEdit;
    eLimiteInferior: TcxCalcEdit;
    eSubsidio: TcxCalcEdit;
    tsPeriodos: TTabSet;
...
begin
  ...
  Try
    ...
    Try
      ...
      cdTablaISR := TZQuery.Create(Self);
      cdHistorial := TZQuery.Create(Self);
...
procedure TFrmSubsidioTablaISPT.tsPeriodosChange(Sender: TObject; NewTab: Integer;
  var AllowChange: Boolean);
begin
  if (Not Llenando) and (tsPeriodos.Tabs.Count > 0) then
  begin
...
begin
  ...
  Try
    ...
    begin
      ...
      if tsPeriodos.Tabs.Count = 1 then
        tsPeriodos.Tabs.Add('* Actual (' + sFecha + ')')
      else
        tsPeriodos.Tabs.Add(sFecha);
...
begin
  Try
    ...
    begin
      ...
      begin
        Buscar := TZReadOnlyQuery.Create(Self);
