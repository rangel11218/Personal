    ...
    Try
      ...
      begin
        ...
        if Not CargarDatosFiltrados(cdVacacionesPeriodos, 'IdTipoNomina,Aplicacion', [TTiponomina(cbTipoNomina1.Items.Objects[cbTipoNomina1.ItemIndex]).identificador, DateToStrSQL(EdtFechaAplicacion.Date)]) then
          raise InteligentException.CreateByCode(6, ['Periodos de Vacaciones', IntToStr(TTiponomina(cbTipoNomina1.Items.Objects[cbTipoNomina1.ItemIndex]).Identificador) + '/' + DateToStrSQL(EdtFechaAplicacion.Date), 'Id. Tipo Nómina/Aplicación']);

        cdVacacionesPeriodos.Close;
        try
          cdVacacionesPeriodos.Open;
          if cdVacacionesPeriodos.RecordCount > 0 then
            raise InteligentException.CreateByCode(29, [':' + #10 + 'Ya existen registros de correspondientes al periodo de ' + DateToStrSQL(EdtFechaAplicacion.Date) + ', si desea agregar registros a este periodo de fechas utilice la opción correspondiente.']);
        finally
          cdVacacionesPeriodos.Close;
        end;
      End;
